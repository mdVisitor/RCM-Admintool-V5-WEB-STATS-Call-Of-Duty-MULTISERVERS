...
$codbox_url = "https://zona-ato-game.ru/codbox/";
...
class serverexec
{
	...
	header('Location: '.$codbox_url.'redirect_sourcebans.php?nick='.$nickname.'&guid='.$steam.'');	  
		  ...
		  if (strpos($reason,'_^1detected ^5http:^5/^5/cyberpub.ru') !== false)
          {
			 $newnamew   = str_replace("_^1detected ^5http:^5/^5/cyberpub.ru", "", $reason); 
			 $newnamew   = str_replace("^1Multi-Hack_", "",  $newnamew);
             $nickname = $newnamew.'_'.$nickname; 
			 $reason = $newnamew."_[ExRecod SA Axon Edition 20k]";
          }			  
