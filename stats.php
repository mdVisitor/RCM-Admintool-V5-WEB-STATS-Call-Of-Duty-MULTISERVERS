...
if ((!empty($search_kills))&&(!empty($server)))
 $re=$bdd ->query("SELECT COUNT(*) AS id FROM db_stats_0 where s_port='$server'");
else if (!empty($server))
 $re=$bdd ->query("SELECT COUNT(*) AS id FROM db_stats_0 where s_port='$server'"); 
else if (!empty($search_geo))
 $re=$bdd ->query("SELECT COUNT(*) AS id FROM db_stats_2 where w_geo='".$search_geo."'");
 else 
{
...
(
    ...
    (
        ...
        ON db_stats_1.s_pg = db_stats_0.s_pg
		        where db_stats_0.s_port=$cntz and db_stats_0.s_lasttime LIKE :keyword and db_stats_1.s_kills >= 1000
...
(
    ...
    (
		...
		INNER JOIN db_stats_0
        ON db_stats_1.s_pg = db_stats_0.s_pg
		        where db_stats_0.s_port=$server and db_stats_0.s_lasttime LIKE :keyword and db_stats_1.s_kills >= 1000
        ORDER BY w_skill DESC
    ) sub0
    CROSS JOIN (SELECT @sort:=0) sub2
) sub1
...
(
    ...
    (
		...
		INNER JOIN db_stats_0
        ON db_stats_1.s_pg = db_stats_0.s_pg
		        where db_stats_1.s_kills >= 1000 and db_stats_0.s_lasttime LIKE :keyword 
        ORDER BY w_skill DESC
    ) sub0
    CROSS JOIN (SELECT @sort:=0) sub2
) sub1
...
if(empty($search_heads)){$shadowheader = $shadow_green; $green_image_shadow = 'lime';}else{$shadowheader = $shadow_red; $green_image_shadow = 'red';}
echo "<td style=\"background:".$colortdztwo.";font-family: Verdana; font-size: 16px;font-size:".$auto_font_title.";".$shadowheader."\"> <center>&emsp;
<span tooltip=\"".$t_accuracy." top\"><a href=\"".$ssylka_na_codbox."stats.php?damage=".$sort_msql_from_headers."&main=". $chatdbarc."&server=".$server."\" style=\"background:".$colortdztwo.";font-family: Verdana; font-size: 16px;font-size:".$auto_font_title.";".$shadowheader."\">
        ...
        <div class="cssProgress">
          <div class="progress3">
            <div class="cssProgress-bar cssProgress-active" data-percent="75" style="width: <?php echo $nextprolvll; ?>;">
              <span class="cssProgress-label">Skill Prestige <?php echo $nextprolvll; ?></span>
            </div>
          </div>
        </div>	
	</td>
        ...
        <div class="cssProgress">
          <div class="progress3">
            <div class="cssProgress-bar cssProgress-success cssProgress-active" data-percent="<?php echo $nextprolvl; ?>" style="width: <?php echo $nextprolvl; ?>;">
              <span class="cssProgress-label"> <?php echo $kills; ?> / Rank <?php echo $nowlelelm; ?> / <?php echo $nextprolvl; ?></span>
            </div>
          </div>
        </div>
		</td>
...
if(!file_exists($crn_bf))
{
	touch($crn_bf);
///////LEVEL 2
$rex=$bdd ->query("SELECT sort, w_skill
FROM
(
    SELECT @sort:=@sort + 1 AS sort, w_skill, s_pg, s_port
    FROM
    (
        SELECT db_stats_2.w_skill, db_stats_2.s_pg, db_stats_2.s_port
        FROM db_stats_2
        INNER JOIN db_stats_1
        ON db_stats_2.s_pg = db_stats_1.s_pg
        WHERE db_stats_2.s_port = $server_port and db_stats_1.s_kills >= 1000
        ORDER BY w_skill DESC
    ) sub0
    CROSS JOIN (SELECT @sort:=0) sub2
) sub1
...
if(!file_exists($crn_bf))
{
	touch($crn_bf);
///////LEVEL 2
$rex=$bdd ->query("SELECT sort, s_kills
FROM
(
    SELECT @sort:=@sort + 1 AS sort, s_kills, s_pg
    FROM
    (
        SELECT db_stats_1.s_kills, db_stats_1.s_pg
        FROM db_stats_1
        INNER JOIN db_stats_0
        ON db_stats_1.s_pg = db_stats_0.s_pg
        WHERE db_stats_0.s_port = $server_port and db_stats_1.s_kills >= 1000
...
if(!file_exists($crn_bf))
{
	touch($crn_bf);
///////LEVEL 2
$rex=$bdd ->query("SELECT sort, s_heads
FROM
(
    SELECT @sort:=@sort + 1 AS sort, s_heads, s_pg, s_port
    FROM
    (
        SELECT db_stats_1.s_heads, db_stats_1.s_pg, db_stats_0.s_port
        FROM db_stats_1
        INNER JOIN db_stats_0
        ON db_stats_1.s_pg = db_stats_0.s_pg
        WHERE db_stats_0.s_port = $server_port and db_stats_1.s_kills >= 1000
...
FROM
(
    SELECT @sort:=@sort + 1 AS sort, s_heads, s_pg
    FROM
    (
        SELECT db_stats_1.s_heads, db_stats_1.s_pg
        FROM db_stats_1
        INNER JOIN db_stats_0
        ON db_stats_1.s_pg = db_stats_0.s_pg
		WHERE db_stats_1.s_kills >= 1000
...
if (is_numeric($key))	
$pageskey = '<a href="'.$ssylka_na_codbox.'stats.php?pass=' . $passsword .'&server=' . $server .'&knife='. $search_knife .'&search=' . $search .'&xgeo=' . $search_geo.'&skill=' . $search_skill.'&headhotsperc=' .$search_heads_percents.'&headhots=' . $search_heads .'&timeh=' . $timesearch .'&time='.$search_time.'&kills=' . $search_kills .'&ratios='. $search_ratiokd .'&deaths=' . $search_deaths.'&page=';   
 else if  ((($search_kills == 'sort') && (!empty($server))))
 {
	$search_kills = '0';
$pageskey = '<a href="'.$ssylka_na_codbox.'stats.php?server=' . $server .'&knife='. $search_knife .'&search=' . $search .'&xgeo=' . $search_geo.'&skill=' . $search_skill.'&headhotsperc=' .$search_heads_percents.'&headhots=' . $search_heads .'&timeh=' . $timesearch .'&time='.$search_time.'&kills=' . $search_kills .'&ratios='. $search_ratiokd .'&deaths=' . $search_deaths.'&page=';
 }
 else
$pageskey = '<a href="'.$ssylka_na_codbox.'stats.php?server=' . $server .'&knife='. $search_knife .'&search=' . $search .'&xgeo=' . $search_geo.'&skill=' . $search_skill.'&headhotsperc=' .$search_heads_percents.'&headhots=' . $search_heads .'&timeh=' . $timesearch .'&time='.$search_time.'&kills=' . $search_kills .'&ratios='. $search_ratiokd .'&deaths=' . $search_deaths.'&page=';
