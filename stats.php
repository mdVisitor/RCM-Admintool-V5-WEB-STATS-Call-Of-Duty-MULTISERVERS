...
foreach ($multi_servers_array as $arx => $xservername) {
   ...
   echo '<li>| <a href="'.$ssylka_na_codbox.'stats.php?server='.$server_md5.'&skill=sort">'.$xservername.'</a></li>';   
}
?>
        </ul>
...
<?php
echo '<a href="'.$ssylka_na_codbox.'stats.php?main='.$chatdbarc.'&skill=sort" style="padding-top: 40px;"> '. $main_servername . '</a>';
...
if(!empty($_SERVER["HTTP_USER_AGENT"])){
 $user_agent = $_SERVER["HTTP_USER_AGENT"];
  ...
  if ((strpos($_SERVER["REQUEST_URI"], '/stats.php?server=') !== false)||(strpos($_SERVER["REQUEST_URI"], '&main=0&server=2') !== false)) {
...
if ((strpos($_SERVER["REQUEST_URI"], '/stats.php?main=0&skill=sort') !== false) || ((strpos($_SERVER["REQUEST_URI"], '&skill=sort') !== false)&&(!empty($server))))
{
...
if(file_exists($stats_db_path_month)){
 	...
 	if (strpos($_SERVER["REQUEST_URI"], '/stats.php?main=0&skill=sort') !== false)
...
////////////////revers 	 
   $findreversed = 0;
if(trim($guidreversed) == trim($guid))
   $findreversed = 1;

unset($arguidn);

if(($findreversed == 1)|| (strlen($guidxport) > 17))
   { 
$colortdzzero = '#800000';
$colortdzone =  '#FF0000';
$guid = "Арвив / Archive".$guid;
...
$color_kills = '#619fc9';
$color_medals = '#7cff00'; 
$color_medals_nom = 'silver'; 
$color_medals_action = 'yellow';
$color_skillers = 'black';
$color_date_timep = 'silver';
$color_date_timepp = 'silver';
$color_nickname = 'silver';
$color_prestige = 'silver';
$color_cip = 'orange';
...
$color_skillers = 'black';
$color_date_timep = 'black';
$color_date_timepp = 'silver';
$color_nickname = 'black';
$color_prestige = 'black';
$color_cip = 'black';;
                                 }
   }		
...
if($sk_sky < max($recordgg))
{
   ...
   if($ilines >= 90){
    ...
    $txt = file_get_contents($h_skf);
    $txt = trim(preg_replace('/[\r\n]+/m',"\n", $txt)); 
    file_put_contents($h_skf, $txt);	
	  

$handle = file($h_skf);
$des = fopen($h_skf, "r+"); 
$destroy = fopen($h_skf, "w+"); 
//убрали верхнюю строку
$a = array_shift($handle);
$hhh = 0;
foreach ($handle as $x => $d) {
	++$hhh;
	if(($hhh -1) == (count($handle)-1))
	{ //echo $hhh; 
         fwrite($des, $d."\n");}
    else 
	   fwrite($des, $d); 	
...
$xl = $bdd->query('SELECT t0.s_pg,t0.s_guid, t0.s_port, t0.servername,t0.s_player,t0.s_time,t0.s_lasttime, t1.name, t1.ip, t1.guid 
 ...
 t1 ON 
 t0.s_guid = t1.guid 
   where t1.ip = "'.$yip.'" and length(t0.s_pg)>4 LIMIT 1');     
...
<b style=\"color:".$color_cip.";text-shadow: -1px -1px 0 #000,-1px 1px 0 #000,1px -1px 0 #000,1px 1px 0 #000;\">
...
<div tooltip=\"".clearnamex($servername)."\">
...
<span tooltip=\"".$servername."\">".colorize($servername)."</span>
</b>
...
<a href=\"".$ssylka_na_codbox."stats.php?search=".$guidspan."&main=".$chatdbarc."&info=min\">
<b style=\"color:".$color_nickname.";text-shadow: 1px 1px 0px black;\">
<span tooltip=".$guid.">".html_entity_decode($playername)."</span></b></a></td>"; 


  if($infomore != 'max')
echo "<td style=\"background:" . ($i % 2 ? $colortdzzero : $colortdzone) . ";opacity: 0.9; font-family: Titillium Web; color: Silver;font-size: 16px;font-size:".$auto_font.";\">&emsp;
<b style=\"color:".$color_kills.";text-shadow: 1px 1px 0px black;\">
<span tooltip=\"($t_series : $n_kills) [ min : $n_kills_min ]\">".$kills."</span></b></td>"; 
   
  if($infomore != 'max')
echo "<td style=\"background:" . ($i % 2 ? $colortdzzero : $colortdzone) . ";opacity: 0.9; font-family: Titillium Web; color: Silver;font-size: 16px;font-size:".$auto_font.";\">&emsp;
<b style=\"color:".$color_deaths.";text-shadow: 1px 1px 0px black;\">
...
if($ratio_skf < $rrattio)
  ...
  if($infomore != 'max')
echo "<td style=\"background:" . ($i % 2 ? $colortdzzero : $colortdzone) . ";opacity: 0.9; font-family: Titillium Web; color: Silver;font-size: 16px;font-size:".$auto_font.";\">&emsp;
<b style=\"color:".$color_kdratio.";text-shadow: 1px 1px 0px black;\">
...
15px; text-shadow: 1px 1px 0px black;\">&emsp;".$procent
."</b>";
 if($infomore != 'max')
...
if (is_numeric($key))	
$pageskey = '<a href="'.$ssylka_na_codbox.'stats.php?pass=' . $passsword .'&server=' . $server .'&knife='. $search_knife .'&search=' . $search .'&killmin=' . $search_kills_min .'&xgeo=' . $search_geo.'&skill=' . $search_skill.'&headhotsperc=' .$search_heads_percents.'&accuaraccyyperc=' .$search_accuaraccyyperc.'&headhots=' . $search_heads .'&timeh=' . $timesearch .'&time='.$search_time.'&kills=' . $search_kills .'&ratios='. $search_ratiokd .'&deaths=' . $search_deaths.'&page=';   
 else if  ((($search_kills == 'sort') && (!empty($server))))
 {
	$search_kills = '0';
$pageskey = '<a href="'.$ssylka_na_codbox.'stats.php?server=' . $server .'&knife='. $search_knife .'&search=' . $search .'&killmin=' . $search_kills_min .'&xgeo=' . $search_geo.'&skill=' . $search_skill.'&headhotsperc=' .$search_heads_percents.'&accuaraccyyperc=' .$search_accuaraccyyperc.'&headhots=' . $search_heads .'&timeh=' . $timesearch .'&time='.$search_time.'&kills=' . $search_kills .'&ratios='. $search_ratiokd .'&deaths=' . $search_deaths.'&page=';
 }
 else
$pageskey = '<a href="'.$ssylka_na_codbox.'stats.php?server=' . $server .'&knife='. $search_knife .'&search=' . $search .'&killmin=' . $search_kills_min .'&xgeo=' . $search_geo.'&skill=' . $search_skill.'&headhotsperc=' .$search_heads_percents.'&accuaraccyyperc=' .$search_accuaraccyyperc.'&headhots=' . $search_heads .'&timeh=' . $timesearch .'&time='.$search_time.'&kills=' . $search_kills .'&ratios='. $search_ratiokd .'&deaths=' . $search_deaths.'&page=';
...
if ($page != 1) $pervpage = $pageskey.'1">'.$t_page_first.'</a> | '.$pageskey.($page - 1).'">'.$t_page_pre.'</a> | ';
// Проверяем нужны ли стрелки вперед
if ($page != $nb_pages) $nextpage = ' | '.$pageskey. ($page + 1) .'">'.$t_page_next.'</a> | '.$pageskey.$nb_pages. '">'.$t_page_last.'</a>';
...
if(!file_exists($cpath."databases/_cached_medals/_cached_medals_".$serverst."/"))
  mkdir($cpath."databases/_cached_medals/_cached_medals_".$serverst."/", 0777, true);	
	
$crn_skill = $cpath."databases/_cached_medals/_cached_medals_".$serverst."/skill.php";
if(!file_exists($crn_skill))touch($crn_skill);

$crn_kills = $cpath."databases/_cached_medals/_cached_medals_".$serverst."/kills.php";
if(!file_exists($crn_kills))touch($crn_kills);

$crn_heads = $cpath."databases/_cached_medals/_cached_medals_".$serverst."/heads.php";
if(!file_exists($crn_heads))touch($crn_heads);

$crn_melle = $cpath."databases/_cached_medals/_cached_medals_".$serverst."/melle.php";
if(!file_exists($crn_melle))touch($crn_melle);

$crn_cobra = $cpath."databases/_cached_medals/_cached_medals_".$serverst."/cobra.php";
if(!file_exists($crn_cobra))touch($crn_cobra);

$crn_n_kills = $cpath."databases/_cached_medals/_cached_medals_".$serverst."/n_kills.php";
if(!file_exists($crn_n_kills))touch($crn_n_kills);	

$crn_n_heads = $cpath."databases/_cached_medals/_cached_medals_".$serverst."/n_heads.php";
if(!file_exists($crn_n_heads))touch($crn_n_heads);

$crn_n_deaths = $cpath."databases/_cached_medals/_cached_medals_".$serverst."/n_deaths.php";
if(!file_exists($crn_n_deaths))touch($crn_n_deaths);

$crn_grenade = $cpath."databases/_cached_medals/_cached_medals_".$serverst."/grenade.php";
if(!file_exists($crn_grenade))touch($crn_grenade);
	
$crn_deaths = $cpath."databases/_cached_medals/_cached_medals_".$serverst."/deaths.php";
if(!file_exists($crn_deaths))touch($crn_deaths);

$crn_suicide = $cpath."databases/_cached_medals/_cached_medals_".$serverst."/suicide.php";
if(!file_exists($crn_suicide))touch($crn_suicide);

$crn_ratio = $cpath."databases/_cached_medals/_cached_medals_".$serverst."/ratio.php";
if(!file_exists($crn_ratio))touch($crn_ratio);

$crn_kill_min = $cpath."databases/_cached_medals/_cached_medals_".$serverst."/kill_min.php";
if(!file_exists($crn_kill_min))touch($crn_kill_min);

$crn_camp = $cpath."databases/_cached_medals/_cached_medals_".$serverst."/camper.php";
if(!file_exists($crn_camp))touch($crn_camp);

$crn_anti_skill = $cpath."databases/_cached_medals/_cached_medals_".$serverst."/anti_skill.php";
...
echo "<hr style=\"height:25px;border:none;color:#222;background-color:#333;\"/>
<center><h1><a href=\"".$ssylka_na_codbox."medals.php\" style=\"text-shadow: 1px 3px 0px black,2px 4px 0px black;\">".$medals."</a></h1>
 <h3><a href=\"".$ssylka_na_codbox."medals.php\" style=\"text-shadow: 1px 3px 0px black,2px 4px 0px black;\">".$medals_pro."</a></h3> 
</center>";
 echo " <table width=\"480 px\" align=\"center\">
<tbody>
<tr>

 
<td style=\"background:" . ($i % 2 ? $colortdzzero : $colortdzone) . ";opacity: 0.9; width: 20%; font-family: Titillium Web; color: ".$color_medals_action.";font-size: 16px;\"><center><img src=\"".$ssylka_na_codbox."img/medals/skill.png\" width=\"80px\"> </br> <b style=\"color: ".$color_medals_nom.";\">$medals_skill</b> </br> 
<a href=\"".$ssylka_na_codbox."stats.php?search=".$skiller_guid."&info=min&main=".$chatdbarc."\" style=\"font-family: Impact; color: ".$color_medals.";font-size: 16px;text-shadow: 1px 3px 0px black,2px 4px 0px black;\">".mb_strimwidth($skiller, 0, 16, "...")."</a> </br> $skill</center></td>
<td style=\"background:" . ($i % 2 ? $colortdzzero : $colortdzone) . ";opacity: 0.9; width: 20%; font-family: Titillium Web; color: ".$color_medals_action.";font-size: 16px;\"><center><img src=\"".$ssylka_na_codbox."img/medals/medal_pro_killer.png\" width=\"80px\"> </br> <b style=\"color: ".$color_medals_nom.";\">$medals_killer</b> </br> 
<a href=\"".$ssylka_na_codbox."stats.php?search=".$killer_guid."&info=min&main=".$chatdbarc."\" style=\"font-family: Impact; color: ".$color_medals.";font-size: 16px;text-shadow: 1px 3px 0px black,2px 4px 0px black;\">".mb_strimwidth($killer, 0, 16, "...")."</a>  </br> $kills</center> </td>
<td style=\"background:" . ($i % 2 ? $colortdzzero : $colortdzone) . ";opacity: 0.9; width: 20%;  font-family: Titillium Web; color: ".$color_medals_action.";font-size: 16px;\"><center><img src=\"".$ssylka_na_codbox."img/medals/medal_pro_headshot.png\" width=\"80px\"> </br> <b style=\"color: ".$color_medals_nom.";\">$medals_headshots</b> </br> 
<a href=\"".$ssylka_na_codbox."stats.php?search=".$headers_guid."&info=min&main=".$chatdbarc."\" style=\"font-family: Impact; color: ".$color_medals.";font-size: 16px;text-shadow: 1px 3px 0px black,2px 4px 0px black;\">".mb_strimwidth($headers, 0, 16, "...")."</a>  </br> $heads</center></td>
<td style=\"background:" . ($i % 2 ? $colortdzzero : $colortdzone) . ";opacity: 0.9; width: 20%;  font-family: Titillium Web; color: ".$color_medals_action.";font-size: 16px;\"><center><img src=\"".$ssylka_na_codbox."img/medals/Rank_Icon_04_platinum.png\" width=\"80px\"> </br> <b style=\"color: ".$color_medals_nom.";\">K/D Ratio</b> </br> 
<a href=\"".$ssylka_na_codbox."stats.php?search=".$ratioxr_guid."&info=min&main=".$chatdbarc."\" style=\"font-family: Impact; color: ".$color_medals.";font-size: 16px;text-shadow: 1px 3px 0px black,2px 4px 0px black;\">".mb_strimwidth($ratioxrpl, 0, 16, "...")."</a> </br>$ratioxr</center></td>
<td style=\"background:" . ($i % 2 ? $colortdzzero : $colortdzone) . ";opacity: 0.9; width: 20%;  font-family: Titillium Web; color: ".$color_medals_action.";font-size: 16px;\"><center><img src=\"".$ssylka_na_codbox."img/medals/medal_pro_knifekills.png\" width=\"80px\"> </br> <b style=\"color: ".$color_medals_nom.";\">$medals_knife</b> </br> 
<a href=\"".$ssylka_na_codbox."stats.php?search=".$knifer_guid."&info=min&main=".$chatdbarc."\" style=\"font-family: Impact; color: ".$color_medals.";font-size: 16px;text-shadow: 1px 3px 0px black,2px 4px 0px black;\">".mb_strimwidth($knifer, 0, 16, "...")."</a> </br> $knifes</center></td>
</tr>
</tbody>
</table> ";
...
<tbody>
<tr>
<td style=\"background:" . ($i % 2 ? $colortdzzero : $colortdzone) . ";opacity: 0.9; width: 20%;  font-family: Titillium Web; color: ".$color_medals_action.";font-size: 16px;\"><center><img src=\"".$ssylka_na_codbox."img/medals/medal_pro_explosivekiller.png\" width=\"80px\"> </br> <b style=\"color: ".$color_medals_nom.";\">".$medals_grenade."</b> </br> 
<a href=\"".$ssylka_na_codbox."stats.php?search=".$grenadekiller_guid."&info=min&main=".$chatdbarc."\" style=\"font-family: Impact; color: ".$color_medals.";font-size: 16px;text-shadow: 1px 3px 0px black,2px 4px 0px black;\">".mb_strimwidth($grenadekiller, 0, 16, "...")."</a> </br> ".$grenade."   </center></td>
<td style=\"background:" . ($i % 2 ? $colortdzzero : $colortdzone) . ";opacity: 0.9; width: 20%;  font-family: Titillium Web; color: ".$color_medals_action.";font-size: 16px;\"><center><img src=\"".$ssylka_na_codbox."img/medals/medal_pro_cobra_20.png\" width=\"80px\"> </br> <b style=\"color: ".$color_medals_nom.";\">".$medals_cobra."</b> </br> 
<a href=\"".$ssylka_na_codbox."stats.php?search=".$cobrakiller_guid."&info=min&main=".$chatdbarc."\" style=\"font-family: Impact; color: ".$color_medals.";font-size: 16px;text-shadow: 1px 3px 0px black,2px 4px 0px black;\">".mb_strimwidth($cobrakiller, 0, 16, "...")."</a> </br>".$cobra."</center></td>


<td style=\"background:" . ($i % 2 ? $colortdzone : $colortdzone) . ";opacity: 0.9; width: 20%;  font-family: Titillium Web; color: ".$color_medals_action.";font-size: 16px;\"><center></br><img src=\"".$ssylka_na_codbox."img/medals/medal_pro_killer_series.png\" width=\"80px\"> </br> <b style=\"color: ".$color_medals_nom.";\">".$medals_series." </br> ".$medals_killer."</b></br>
<a href=\"".$ssylka_na_codbox."stats.php?search=".$n_killkiller_guid."&info=min&main=".$chatdbarc."\" style=\"font-family: Impact; color: ".$color_medals.";font-size: 16px;text-shadow: 1px 3px 0px black,2px 4px 0px black;\">".mb_strimwidth($n_killkiller, 0, 16, "...")."</a> </br> ".$n_kills."</center></td>

<td style=\"background:" . ($i % 2 ? $colortdzone : $colortdzone) . ";opacity: 0.9; width: 20%;  font-family: Titillium Web; color: ".$color_medals_action.";font-size: 16px;\"><center></br><img src=\"".$ssylka_na_codbox."img/medals/medal_pro_killer_minute.png\" width=\"80px\"> </br> <b style=\"color: ".$color_medals_nom.";\">".$medals_series."</b>/<b style=\"color: red;\">min</b> </br> <b style=\"color: ".$color_medals_nom.";\"> ".$medals_killer."</b> </br> 
<a href=\"".$ssylka_na_codbox."stats.php?search=".$n_kills_min_guid."&info=min&main=".$chatdbarc."\" style=\"font-family: Impact; color: ".$color_medals.";font-size: 16px;text-shadow: 1px 3px 0px black,2px 4px 0px black;\">".mb_strimwidth($n_kills_minr, 0, 16, "...")."</a> </br> ".$n_kills_min."</center></td>

<td style=\"background:" . ($i % 2 ? $colortdzone : $colortdzone) . ";opacity: 0.9; width: 20%;  font-family: Titillium Web; color: ".$color_medals_action.";font-size: 16px;\"><center></br><img src=\"".$ssylka_na_codbox."img/medals/medal_pro_headshot_series.png\" width=\"80px\"> </br> <b style=\"color: ".$color_medals_nom.";\">".$medals_series." </br> ".$medals_headshots."</b> </br> 
<a href=\"".$ssylka_na_codbox."stats.php?search=".$n_headers_guid."&info=min&main=".$chatdbarc."\" style=\"font-family: Impact; color: ".$color_medals.";font-size: 16px;text-shadow: 1px 3px 0px black,2px 4px 0px black;\">".mb_strimwidth($n_headers, 0, 16, "...")."</a> </br> ".$n_heads."</center></td>
...
echo "<center><h3><a href=\"".$ssylka_na_codbox."medals.php\" style=\"text-shadow: 1px 3px 0px black,2px 4px 0px black;\">".$medals_anti."</a></h3></center>
...
<tr>
<td style=\"background:" . ($i % 2 ? $colortdzzero : $colortdzone) . ";opacity: 0.9; width: 20%;  font-family: Titillium Web; color: ".$color_medals_action.";font-size: 16px;\"><center></br><img src=\"".$ssylka_na_codbox."img/medals/anti_skill.png\" width=\"80px\"> </br> <b style=\"color: ".$color_medals_nom.";\">Anti $medals_skill</b></br> 
<a href=\"".$ssylka_na_codbox."stats.php?search=".$skiller_anti_guid."&info=min&main=".$chatdbarc."\" style=\"font-family: Impact; color: ".$color_medals.";font-size: 16px;text-shadow: 1px 3px 0px black,2px 4px 0px black;\">".mb_strimwidth($skiller_anti, 0, 16, "...")."</a> </br>$skill_anti</center></td>


<td style=\"background:" . ($i % 2 ? $colortdzzero : $colortdzone) . ";opacity: 0.9; width: 20%;  font-family: Titillium Web; color: ".$color_medals_action.";font-size: 16px;\"><center></br><img src=\"".$ssylka_na_codbox."img/medals/medal_anti_no1target_series.png\" width=\"80px\"> </br> <b style=\"color: ".$color_medals_nom.";\">$medals_series $medals_deaths</b></br> 
<a href=\"".$ssylka_na_codbox."stats.php?search=".$n_deaths_guid."&info=min&main=".$chatdbarc."\" style=\"font-family: Impact; color: ".$color_medals.";font-size: 16px;text-shadow: 1px 3px 0px black,2px 4px 0px black;\">".mb_strimwidth($n_deaths, 0, 16, "...")."</a> </br>$n_death</center></td>

<td style=\"background:" . ($i % 2 ? $colortdzzero : $colortdzone) . ";opacity: 0.9; width: 20%;  font-family: Titillium Web; color: ".$color_medals_action.";font-size: 16px;\"><center></br><img src=\"".$ssylka_na_codbox."img/medals/medal_anti_no1target.png\" width=\"80px\"> </br><b style=\"color: ".$color_medals_nom.";\">$medals_deaths</b></br> 
<a href=\"".$ssylka_na_codbox."stats.php?search=".$antideaths_guid."&info=min&main=".$chatdbarc."\" style=\"font-family: Impact; color: ".$color_medals.";font-size: 16px;text-shadow: 1px 3px 0px black,2px 4px 0px black;\">".mb_strimwidth($antideaths, 0, 16, "...")."</a> </br>$s_deaths</center></td>

<td style=\"background:" . ($i % 2 ? $colortdzzero : $colortdzone) . ";opacity: 0.9; width: 20%;  font-family: Titillium Web; color: ".$color_medals_action.";font-size: 16px;\"><center></br><img src=\"".$ssylka_na_codbox."img/medals/suicidemaset.png\" width=\"80px\"> </br> <b style=\"color: ".$color_medals_nom.";\">$medals_suicides</b></br> 
<a href=\"".$ssylka_na_codbox."stats.php?search=".$suicider_guid."&info=min&main=".$chatdbarc."\" style=\"font-family: Impact; color: ".$color_medals.";font-size: 16px;text-shadow: 1px 3px 0px black,2px 4px 0px black;\">".mb_strimwidth($suicider, 0, 16, "...")."</a> </br>$suicids</center></td>

<td style=\"background:" . ($i % 2 ? $colortdzzero : $colortdzone) . ";opacity: 0.9; width: 20%;  font-family: Titillium Web; color: ".$color_medals_action.";font-size: 16px;\"><center></br><img src=\"".$ssylka_na_codbox."img/medals/medal_anti_caper.png\" width=\"80px\"> </br> <b style=\"color: ".$color_medals_nom.";\">CAMP</b></br> 
<a href=\"".$ssylka_na_codbox."stats.php?search=".$camp_guid."&info=min&main=".$chatdbarc."\" style=\"font-family: Impact; color: ".$color_medals.";font-size: 16px;text-shadow: 1px 3px 0px black,2px 4px 0px black;\">".mb_strimwidth($campl, 0, 16, "...")."</a> </br>$campr</center></td>
...
if (($_GET['theme']) == 'light') 
  echo '<a href="'.$ssylka_na_codbox.'stats.php?theme=dark" title="Вернуться к началу" class="topbutton">Style</a>';
else
  echo '<a href="'.$ssylka_na_codbox.'stats.php?theme=light" title="Вернуться к началу" class="topbuttondark">Style</a>';
